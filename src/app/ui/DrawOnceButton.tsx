...
export default function DrawOnceButton({
  candidates,
  prizes,
  drawRules,
  results,
  ...
  const handleDraw = () => {
    ...
    const availableCandidates = candidates.filter(
      (name) => !winnerSet.has(name),
    );
    const availablePrizes: Prize[] = prizes
      .flatMap((p) => {
        const cnt =
          p.count -
          results.reduce((cnt, r) => (r.prize == p.name ? cnt + 1 : cnt), 0);
        return cnt > 0 ? { name: p.name, count: cnt } : undefined;
      })
      .filter((p): p is Prize => p !== undefined);
    const cntAvailablePrizes = availablePrizes.reduce(
      (sum, p) => sum + p.count,
      0,
    );
    ...
    const minPrizesValidation = () => cntAvailablePrizes == 0;
    ...
    const drawResults = draw(
      ...
      [
        {
          name:
            drawRules.order === "first"
              ? availablePrizes[0].name
              : availablePrizes[availablePrizes.length - 1].name,
          count: 1,
        },
      ],
      drawRules,
